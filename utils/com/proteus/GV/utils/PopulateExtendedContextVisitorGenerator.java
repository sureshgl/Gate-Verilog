...
public class PopulateExtendedContextVisitorGenerator {
	...
	private static void generateTestClasses(Map<String, RuleType> rules) throws FileNotFoundException {
		...
		sb.append("public class PopulateExtendedContextVisitor extends GV_extBaseVisitor<ParserRuleContext>{\n");
		for(Entry<String, RuleType> e:rules.entrySet()){
			if(! e.getValue().isAlt()){
				count++;
		...
		sb.append("}");
		FileUtils.WriteFile(new File("tools/com/proteus/GV/PopulateExtendedContextVisitor.java"), sb);
