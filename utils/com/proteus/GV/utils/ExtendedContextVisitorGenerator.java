...
public class ExtendedContextVisitorGenerator {
	...
	private static void generateTestClasses(Map<String, RuleType> rules) throws FileNotFoundException {
		...
		sb.append("import com.proteus.GV.ext.AbstractBaseExt;\n");
		sb.append("import com.proteus.GV.ANTLRv4.gen.GV_extBaseVisitor;\n");
		sb.append("import com.proteus.GV.ANTLRv4.gen.GV_extParser;\n\n");
		
		sb.append("public class ExtendedContextVisitor extends GV_extBaseVisitor<AbstractBaseExt>{\n");
		for(Entry<String, RuleType> e:rules.entrySet()){
			if(! e.getValue().isAlt()){
				count++;
				String newName = e.getKey().substring(0, 1).toUpperCase() + e.getKey().substring(1);
				sb.append("	@Override public AbstractBaseExt visit"+newName+"(GV_extParser."+newName+"Context ctx) {\n");
		...
		sb.append("}");
		FileUtils.WriteFile(new File("tools/com/proteus/GV/ExtendedContextVisitor.java"), sb);
